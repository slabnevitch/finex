...
export default {
    ...
    async FETCH_LIST({commit}, payload = {queryParams: null}) {
        ...
        const response = await requestFunctions.getDataAsync(urls.news_category.GET + requestFunctions.generateQueryString(payload.queryParams));
    ...
    async FETCH_ITEM({commit}, payload) {
        let url = requestFunctions.replacePlaceholderQueryString(urls.news_category.GET_ITEM, '{id}', payload.id);
        ...
        if (responseObj.success === true) {
            ...
            //
            responseObj.message = 'Ошибка при получении данных #GET_ITEM_NEWS_CATEGORY';
        }
    ...
    async DELETE({commit}, payload) {
        let url = requestFunctions.replacePlaceholderQueryString(urls.news_category.DELETE, '{id}', payload.id);
        // отправляем запрос
        ...
        if (responseObj.success === true) {
            ...
            //
            responseObj.message = 'Ошибка при отправке данных #DELETE_NEWS_CATEGORY';
        }
    ...
    async CREATE({commit, getters}, payload) {
        ...
        // отправляем запрос
        const response = await requestFunctions.postDataAsync__json(urls.news_category.CREATE, post_data__serialized);
        ...
        if (responseObj.success === true) {
            ...
            //
            responseObj.message = 'Ошибка при отправке данных #CREATE_NEWS_CATEGORY';
        }
    ...
    async UPDATE({commit, getters}, payload) {
        let url = requestFunctions.replacePlaceholderQueryString(urls.news_category.UPDATE, '{id}', payload.id);
        let post_data__serialized = getters['SERIALIZE_DATA'](payload);
        // отправляем запрос
        ...
        if (responseObj.success === true) {
            ...
            //
            responseObj.message = 'Ошибка при отправке данных #UPDATE_NEWS_CATEGORY';
        }
