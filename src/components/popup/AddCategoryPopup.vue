...
<script>
    ...
    export default {
        ...
        computed: {
            flag_open__computed() {
                let flag_open = this.$store.getters['popup.popup_service_category_form/GET_STATUS_OPEN'];

                if(flag_open == true){
                    let popup_data = this.$store.getters['popup.popup_service_category_form/GET_DATA'];
        ...
        methods: {
            async save() {
                ...
                let response_obj = await this.$store.dispatch('popup.popup_service_category_form/SAVE', send_date);
